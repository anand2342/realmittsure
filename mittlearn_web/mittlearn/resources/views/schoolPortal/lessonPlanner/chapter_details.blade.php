...
@section('content')
    ...
    <div class="cardBox myCourses">
        ...
        <div class="classSubjectBookName mb-1 d-flex justify-content-between align-items-center">

            <span>
                {{ $className }} - {{ $subjectName }} - {{ $courseName->course_name }}
            </span>

            @if ($courseName->ext_file)
                <a href="{{ Storage::url($courseName->ext_file) }}" target="_blank">
                    <button class="btn btn-primary-gradient rounded-1 addBtn">
                         <i class="bi bi-file-earmark-text me-1"></i>TRM / Lesson Plan
                    </button>
                </a>
            @endif
