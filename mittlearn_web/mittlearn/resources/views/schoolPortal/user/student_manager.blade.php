        ...
        <div class="row">
            <div class="col-md-8 mb-3">
                <div class="teacherLeft">
                    ...
                    <a href="{{ route('sp.student.add') }}" class="btn btn-primary-gradient rounded-1 addBtn">Add
                        Student</a>
                    @if (getUserRoles() == 'school_admin')
                        <a href="{{ route('sp.check.student.access') }}"
                            class="btn btn-primary-gradient rounded-1 addBtn">Login Access Details</a>
                        <a href="{{ route('sp.un-verfired.student') }}"
                            ...
                            <tbody id="studentTableBody">
                                @foreach ($students as $student)
                                    <tr data-id="{{ $student->id }}"
                                        ...
                                        data-parent_name="{{ $student->studentDetails->parent_name ?? null }}"
                                        data-admission_date="{{ \Carbon\Carbon::parse($student->studentDetails->doj ?? null)->format('Y-m-d') }}"
                                        data-class="{{ $student->studentDetails->class ?? null }}"
                                        data-section="{{ $student->studentDetails->section ?? null }}"
                                        data-parent_mobile_no="{{ $student->mobile_no }}"
                                        data-status="{{ $student->status == 1 ? 'active' : 'inactive' }}"
                                        data-dob="{{ $student->studentDetails->dob ?? null }}">
                                        <td>{{ $student->userAdditionalDetail->admission_no ?? null }}</td>
                                        <td>
                                        ...
                                        <td>{{ \Carbon\Carbon::parse($student->studentDetails->doj ?? null)->format('d/m/Y') }}
                                        </td>
                                        <td>{{ App\Models\SchoolClass::where('id', $student->studentDetails->class ?? null)->value('name') }}
                                        </td>
                                        @if (config('COURSES_FILTER_BY_ACCESS_CODE') == 1)
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a class="dropdown-item viewEditStudent edit-student-btn"
                                                            id="edit-student-btn"
                                                            href="{{ route('sp.student.edit', $student->id) }}">View/Edit
                                                            Student
                                                            Details</a>
                                                    </li>
                                                    <li>
                                                    ...
                                                    <li><a class="dropdown-item" href="#logsView"
                                                            data-id="{{ $student->id }}" data-bs-toggle="offcanvas">View
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group bginput mb-3">
                            ...
                            {!! Form::select('section', $sections, old('section', $userData->studentDetails->section ?? null), [
