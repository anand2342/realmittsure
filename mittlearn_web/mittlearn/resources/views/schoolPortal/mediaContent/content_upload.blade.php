            ...
            <div id="search-results" class="row mt-3 mb-2">
                @if ($mittlearnFolderListing && $mittlearnFolderListing->isNotEmpty())
                    @foreach ($mittlearnFolderListing as $data)
                        <div class="col-md-4 col-xl-3 px-2 mb-3 class-item" data-title="{{ $data->folder_name }}"
                            title="{{ $data->folder_name }}">

                            <a href="{{ route('content.folder.view', $data->id) }}"
                                class="digitaluplBox h-100 d-flex flex-column p-3"
                                style="background-color: {{ $data->folder_color }}; border-radius: 12px;">
                                ...
                                <div class="text-center my-2">
                                    <img src="{{ asset($data->folder_icon) }}" alt="" style="height: 55px;">
                                </div>

                                <!-- BOTTOM: Folder Name + Files -->
                                <div class="text-center mt-2">
                                    <div class="folder-name fw-semibold" style="color: #000; font-size: 14px;">
                                        {{ Str::limit($data->folder_name, 18, '...') }}
                                    </div>
                                    <small class="text-muted">
                                        {{ $data->file_count_count }} Files
                                    </small>
                                </div>
