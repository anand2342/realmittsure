        ...
        <div class="d-md-flex justify-content-between align-items-center mb-3">
            <div class="headingList my-3 mb-4">
                ...
                <div>
                    ...
                    <p>
                        Explore the subjects with detailed digital content, assignments, and interactive materials to
            ...
            @if (config('COURSES_FILTER_BY_ACCESS_CODE') == 1)
                @if (getUserRoles() !== 'school_teacher')
                    <div class="plannerHeader">
                        <a class="btn btn-primary-gradient rounded-1 py-2 mt-2"
                            href="{{ route('sp.access.codes', $classId) }}">Access
                            Code</a>
                    </div>
                @endif
            @endif
            ...
            @foreach ($subjects as $index => $data)
                <div class="col-md-4 col-lg-3 mb-3 px-md-2">
                    <div class="languageBox subjectListDiv h-100 postion-relative pt-0">
                        ...
                        <p style="min-height:20px" class="fw-semibold text-center mt-2">{{ $data->course_name ?? ' ' }}</p>
                        <!-- Course Name -->
                       
                        <div class="d-xxl-flex justify-content-between gap-2 align-items-center">
                            <a href="{{ route('sp.course.listing', [$data->id, $classId]) }}"
                                class="btn btn-primary-gradient rounded-1 py-2 w-100 mb-2">Explore</a>
                            @if ($data->course_id)
                                <a href="{{ route('sp.courses.details', ['id' => $data->course_id, 'classId' => $classId, 'subjectId' => $data->id]) }}"
