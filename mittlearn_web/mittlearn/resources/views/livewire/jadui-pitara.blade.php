<div>
    <div class="card">
        <div class="card-body">
            <h5 class="card-title mb-0">Jadui Pitara Assigned Classes</h5>
            <hr class="form-divider">

            {{ Form::open(['url' => route('jadui.pitara.class.update'), 'id' => 'edit-plan-form', 'class' => 'row g-3', 'enctype' => 'multipart/form-data']) }}
    ...
    <div class="row">
        <div class="col-lg-12">
            {{ Form::open(['url' => route('jadui.pitara.series'), 'id' => 'add-plan-form', 'class' => 'row g-3']) }}
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach ($allClasses as $classId => $className)
                                @if (in_array($classId, $assignedClasses))
                                            ...
                                            <table class="table">
                                                @foreach ($rows[$classId] as $index => $row)
                                                    <tr wire:key="row-{{ $classId }}-{{ $index }}">
                                                        ...
                                                        <td width="45%">
                                                            <select x-data="select2"
                                                                class="js-select2 form-select" multiple="multiple"
                                                                placeholder="Select"
