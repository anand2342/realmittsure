    ...
    <div class="loginMain">
                    ...
                    <form method="POST" action="{{ route('store.qr.register') }}" id="register-password-form">
                        ...
                        <div class="row px-md-1">
                            ...
                            <div id="classSelectWrapper" class="col-md-6 px-md-2 form-group mb-3" style="display: none;">
                                ...
                                <select name="className" required id="className"
                            ...
                            <div id="seriesSelectWrapper" class="col-md-6 px-md-2 form-group mb-3" style="display: none;">
                                ...
                                <select name="seriesName" required id="seriesName"
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                const seriesSelectWrapper = $("#seriesSelectWrapper");
                const subjectSelectWrapper = $("#subjectSelectWrapper");
                const classSelect = $("#className");
                const seriesSelect = $("#seriesName");
                const subjectSelect = $("#subjectSelect");

                const schools = JSON.parse(document.getElementById("schools-data").dataset.schools);
                ...
                $(document).on("click", function(e) {
                    if (!$(e.target).closest("#schoolNameSearch, #schoolSearchResults").length) {
                        schoolSearchResults.hide();
                    }
                });
                schoolSearchInput.on("blur", function() {
                    setTimeout(() => {
                        if (schoolSelect.val() === "" && schoolSearchInput.val() !== "") {
                            ...
                            seriesSelectWrapper.show();
                            subjectSelectWrapper.show();
                            resetClassSelect();
                            resetSeriesSelect();
                            resetSubjectSelect();
                        }
                ...
                schoolSelect.on("change", function() {
                    ...
                    // Show class & series; hide subject until both chosen
                    classSelectWrapper.show();
                    seriesSelectWrapper.show();
                    subjectSelectWrapper.hide();

                    // Reset class, series, subject
                    resetClassSelect();
                    resetSeriesSelect();
                    resetSubjectSelect();
                });
                ...
                // ── Try fetch subjects when class + series both selected ───────────────
                function tryFetchSubjects() {
                    const schoolId = schoolSelect.val(); // may be empty if custom school
                    const classId = classSelect.val();
                    const seriesId = seriesSelect.val();

                    if (!classId || !seriesId) return; // need at least class + series

                    fetchSubjects(schoolId, classId, seriesId);
                }

                // ── AJAX: fetch subjects ───────────────────────────────────────────────
                function fetchSubjects(schoolId, classId, seriesId) {
                    $.ajax({
                        url: "/get-subjects",
                        method: "GET",
                        data: {
                        ...
                        success: function(response) {
                            ...
                            if (response.subjects && Object.keys(response.subjects).length > 0) {
                                Object.entries(response.subjects).forEach(function([id, name]) {
                ...
                function resetClassSelect() {
                    classSelect.empty().append('<option value="" selected>Select Class</option>');
                    // Re-add blade-rendered options
                    @foreach ($classes as $id => $name)
                ...
                function resetSeriesSelect() {
                    seriesSelect.empty().append('<option value="" selected>Select Series</option>');
                    @foreach ($series as $id => $name)
                        seriesSelect.append('<option value="{{ $id }}">{{ $name }}</option>');
                    @endforeach
                }

                function resetSubjectSelect() {
                    subjectSelect.empty().append('<option value="" selected>Select Subject</option>');
                }
                ...
                function resetFromClass() {
                    ...
                    seriesSelectWrapper.hide();
                    subjectSelectWrapper.hide();
                }

                // ── Custom school name on form submit ──────────────────────────────────
                $("form").on("submit", function() {
                    if (schoolSelect.val() === "" && schoolSearchInput.val().trim() !== "") {
                        $("<input>").attr({
                            type: "hidden",
                            name: "customSchoolName",
                            value: schoolSearchInput.val().trim()
                        }).appendTo("form");
                    }
                });
