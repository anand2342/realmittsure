                                ...
                                <tbody>
                                    @foreach ($data as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->name }}</td>
                                            <td>
                                                {{ $item->landing_ui == 'mittlearn' ? 'Mittlearn' : ($item->landing_ui == 'mittbunny' ? 'MittBunny' : 'Unknown Status') }}
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @isPermission('class.edit')
                                                    <a class="btn btn-sm btn-warning"
                                                        href="{{ route('class.edit', $item->id) }}"><i
                                                            class="fa fa-pencil"></i></a>
                                                @endisPermission
                                                {{--  @isPermission('class.delete')
                                                    <a class="btn btn-danger btn-sm me-2" href="javascript:void(0);"
                                                        onclick="confirmDelete('{{ route('class.delete', $item->id) }}')">
