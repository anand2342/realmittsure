                                    ...
                                    <tbody>
                                        @foreach ($datalist as $k => $val)
                                            <tr>
                                                ...
                                                <td>{{ $val->bookSeries->name }}</td>
                                                <td>{{ $val->class }}</td>
                                                <td>{{ $val->subject }}</td>
                                                <td> <span
                                                        class="badge {{ $val->is_active ? 'text-success' : 'text-danger' }}">
                                                        {{ config('constants.STATUS_LIST')[$val->is_active] ?? 'Unknown Status' }}
                                                    </span></td>
                                                <td>

                                                    {{--  @isPermission('course-bucket.edit')  --}}
                                                        <a class="btn btn-sm btn-warning"
                                                            href="{{ route('course-bucket.edit', [$val->id]) }}"><i
                                                                class="fa fa-pencil"></i></a>
                                                    {{--  @endisPermission  --}}
                                                    {{--  @isPermission('course-bucket.delete')  --}}
                                                        <button class="btn btn-danger btn-sm delete_btn"
                                                            data-url="{{ route('course-bucket.delete', [$val->id]) }}"
