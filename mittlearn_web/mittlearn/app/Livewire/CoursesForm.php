...
class CoursesForm extends Component
{
    ...
    public function mount($category,  $course = null,  $modelsData = null, $metadataFieldValues = null, $seriesId = null)
    {
        ...
        $this->priceType  = ['paid' => 'Paid', 'free' => 'Free'];
        $this->type = $course->price_type ?? 'paid';
    ...
    public function priceTypeChange() {}
    ...
    public function loadConstants($categoryId)
    {
        ...
        if ($category) {
            ...
            $this->childCategories = $category->where('status', 1)->where('parent_id', $categoryId)->pluck('name', 'id')->toArray();
            ...
            if ($category->id == '2') {
                $this->courseMetadataFields = CourseMetadataField::where('is_active', 1)->where('category_id', '2')->orderBy('sort_order', 'asc')->get();
                $this->showBoardSelect = true;
            }
    ...
    public function getMetaDataByCategoryId()
    {
        if ($this->selectedSubCategory) {
            if ($this->selectedCategory == '1') {
                $this->courseMetadataFields = CourseMetadataField::where('is_active', 1)->where('category_id', $this->selectedSubCategory)->orderBy('sort_order', 'asc')->get();
            } else {
                $this->courseMetadataFields = CourseMetadataField::where('is_active', 1)->where('category_id', '2')->orderBy('sort_order', 'asc')->get();
            }
        } else {
