...
{
    public $id, $board_id, $medium_id, $series_id, $class_id, $subject_id, $question_types, $question_type,  $boards, $mediums, $bookSeries, $classes, $subjects, $chapters = [], $chapter_ids = [], $chapter_id = [], $seleted_chapter_id = [], $question, $description, $marks, $difficulty_level, $questions;
    public $answer;
    ...
    public function mount($question = null)
    {
        ...
        if ($question) {
            ...
            $this->series_id = $question->series_id;
            $this->class_id = $question->class_id;
            $this->subject_id = $question->subject_id;
            if ($this->board_id && $this->medium_id) {
                $this->bookSeries = BookSeries::where('board_id', $this->board_id)->where('medium_id', $this->medium_id)->pluck('name', 'id')->toArray();
            }
            if ($this->series_id) {
                $bookSeries = BookSeries::find($this->series_id);
                if ($bookSeries) {
            ...
            if ($this->class_id) {
                $selectedClassSubjects = collect($this->classSubjectsMapping)->firstWhere('class_id', $this->class_id);
            ...
            if ($this->series_id && $this->class_id && $this->subject_id) {
                $courses = Course::query();

                if ($this->board_id && $this->board_id != 0) {
                    $courses->whereHas('metadataValues', function ($query) {
                        $query->where('field_name', 'board')
                            ->where('field_value', $this->board_id);
                    });
                }

                if ($this->medium_id && $this->medium_id != 0) {
                    $courses->whereHas('metadataValues', function ($query) {
            ...
            $this->chapter_ids = CourseChapter::whereIn('course_id', $courseIds)->pluck('chapter_name', 'id')->toArray();
            $this->selectedChapters = explode(',', $question->chapter_id);
    ...
    public function getBookSeries($boardId, $mediumId)
    {
        ...
        $query = BookSeries::query()->where('is_active', 1);
        if (!empty($this->board) && $this->board != 0) {
            $query->where('board_id', $this->board);
        }
        if (!empty($this->medium) && $this->medium != 0) {
    ...
    {
        $this->board = $this->board_id;
        $this->medium = $this->medium_id;

        if ($this->series_id && $this->class_id && $this->subject_id) {
            $courses = Course::query();

            // Conditionally apply board filter
            if ($this->board && $this->board != 0) {
                $courses->whereHas('metadataValues', function ($query) {
                    $query->where('field_name', 'board')
                        ->where('field_value', $this->board);
                });
            }

            // Conditionally apply medium filter
            if ($this->medium && $this->medium != 0) {
                $courses->whereHas('metadataValues', function ($query) {
                    $query->where('field_name', 'medium')
                        ->where('field_value', $this->medium);
                });
            }

            // Always apply series, class, subject
            $courses->whereHas('metadataValues', function ($query) {
                $query->where('field_name', 'series')
                    ->where('field_value', $this->series_id);
            })->whereHas('metadataValues', function ($query) {
            ...
            $this->chapter_ids = CourseChapter::whereIn('course_id', $courseIds)
    ...
    // #[On('chapterUpdated')]
    // public function updatedChapterFromJS($value)
    // {
    ...
    public function save($id = null)
    {
        ...
        try {
            ...
            if (!empty($this->chapter_id)) {
                $data['chapter_id'] = implode(',', $this->chapter_id);
            }
            ...
            if ($this->question_type === 'match-the-following') {
                ...
                foreach (array_slice($this->matchTheFollowing, 0, 4) as $index => $option) {
                    $leftOptions[] = $question_bank->options()->updateOrCreate(
                        ...
                        [
                            'option_text' => strip_tags($option['option_match'], '<img>'), // Keep <img>
                            'is_correct'  => false,
                        ]
                    );
                }
