...
class LoginController extends Controller
{
    ...
    public function loginSubmit(Request $request)
    {
        ...
        if (preg_match($emailPattern, $credentials['username'])) {
            $field = 'email';
            $value = $credentials['username'];
        } elseif (preg_match($mobilePattern, $credentials['username'])) {
            $field = 'mobile_no';
            $value = $credentials['username'];
            ...
            return redirect()->route('login')->with(['error' => 'The username must be a valid Email or Mobile Number.']);
        }
        $remember = $request->has('remember');

        // Now try to authenticate the user without logging them in yet
        if (Auth::validate([$field => $value, 'password' => $credentials['password']])) {

            // User exists and credentials are correct
            $user = User::where($field, $value)->first();
            if (! $user) {
                return redirect()->route('login')->with(['error' => 'User not found.Please enter valid Email or Mobile Number.']);
            }
            if ($user->status == 0) {
            ...
            $this->mapQrContentToUser($request, $user);
            $this->mapTandSQrContentToUser($request, $user);


            if ($user->is_email_verified == 0 && preg_match($emailPattern, $value)) {
                // Email not verified: Generate OTP and redirect to verification page
                ...
                // session(['otp_value' => $otp, 'id' => $user->id]);
                $otpSession->otp = $otp;
                // $otpSession->otp_verified = '1';
                $otpSession->mobile_email = $user->email;
                $otpSession->expire_at    = now()->addMinutes(10);
                $otpSession->save();

                // dd($otp);

                return view("auth.email-verify", ['data' => $user->email]);
            } else if ($user->is_mobile_verified == 0 && preg_match($mobilePattern, $value)) {
                $otpSession = OtpSession::where('session_id', $user->id)->where('otp_verified', 0)->where('expire_at', '>', now())->orderBy('created_at', 'desc')->first();
                if (! $otpSession) {
                    $otpSession             = new OtpSession;
                    $otpSession->session_id = $user->id;
                ...
                // If email is already verified, log the user in
                Auth::login($user);
        ...
        if ($request->filled('class_id') || $request->filled('category_id')) {
            return redirect()->back()->with(['error' => 'Login details are incorrect']);
        }
        return redirect()->route('login')->with(['error' => 'Login details are incorrect']);
    }
    private function mapQrContentToUser(Request $request, User $user)
    {
        // If login is NOT coming from QR, silently skip
        if (! $request->filled('class_id') || ! $request->filled('category_id')) {
            return;
        }
    ...
    private function mapTandSQrContentToUser(Request $request, User $user)
    {
        // If login is NOT from Talent-Skills QR
        if (!$request->filled('course_ids') || !$request->filled('sub_category_id')) {
            return;
        }
    ...
    public function logout()
    {
        ...
        Session::forget('student_overview');
        Session::forget('student_class');
