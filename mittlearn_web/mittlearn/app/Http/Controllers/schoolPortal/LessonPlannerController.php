...
class LessonPlannerController extends Controller
{
    ...
    public function chapterDetails($id, $classId, $subjectId)
    {
        ...
        try {
            ...
            $this->data['courseName'] = Course::where('id', $id)->where('is_active', 1)->first();
    ...
    public function chapterPlanner($id, $course_id, $subject_id, $class_id)
    {
        try {
            ...
            if (! $plannerLesson) {
                return redirect()->back()->with(['error' => '🔍 No records found. It appears the lesson planner is currently being added by the Mittlearn team. Please try again after some time. 🧐',]);
            }
