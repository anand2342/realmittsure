...
class CoreController extends Controller
{
    ...
    // As per ticket# Ticket#164 - id 3=Mathematics, 4=Science, 25=Mathematics (EL) , 26=Science (EL) |  
    private const REASONING_REQUIRED_SUBJECTS = [3, 4, 25, 26];
        ...
        if ($request->from == "web") {
            ...
            if ($userClass) {
                if (getUserRoles() === 'd2c_user') {
                    ...
                    foreach ($userClasses as $userClass) {
                        ...
                        if ($groupId == '35') {
                            ...
                            foreach ($userAccessCodeAssignedCourses as $accessCode) {
                                ...
                                $key = $accessCode->class_id . '-' . $accessCode->book_series_id;

                                if (!isset($subjectMap[$key])) {
                                    $subjectMap[$key] = [];
                                }

                                $subjectMap[$key][] = $accessCode->subject_id;
                            }
                            ...
                            foreach ($userAccessCodeAssignedCourses as $accessCode) {
                                ...
                                $showReasoning =
                                    count(array_intersect(
                                        self::REASONING_REQUIRED_SUBJECTS,
                                        $subjectMap[$key] ?? []
                                    )) > 0;
                                // foreach (self::REASONING_REQUIRED_SUBJECTS as $pair) {
                                //     if (
                                //         in_array($pair[0], $subjectMap[$key] ?? []) &&
                                //         in_array($pair[1], $subjectMap[$key] ?? [])
                                //     ) {
                                //         $showReasoning = true;
                                //         break;
                                //     }
                            ...
                            $assignedCourses = D2CDigitalContent::where('class_id', $classId)
                                ->where('sn', $userClass->sn) // Filter by SN for multiple qr for same class
                    ...
                    // ================= Talent/Skill QR Courses =================
                    $talentSkillCourses = TalentSkillQrCourse::where('user_id', $userId)
                        ->where('user_role', 'd2c_user')
                        ->where('category_id', 2) // Non-academic
                        ->get();

                    foreach ($talentSkillCourses as $talentCourse) {
                        $courseIds = explode(',', $talentCourse->course_ids);
            ...
            if ($userClass) {
                ...
                if ($now->diffInDays($createdAt) < 15) {
                    ...
                    foreach ($subscriptionPurchase as $purchase) {
                        ...
                        $academicCourseIds = array_column(
                            $coursesData['academic_courses'] ?? [],
                            'id'
                        );

                        $nonAcademicCourseIds = array_column(
                            $coursesData['non_academic_courses'] ?? [],
                            'id'
                        );

                        if (!empty($academicCourseIds)) {
                            $res['academic_courses'] = Course::whereIn('id', $academicCourseIds)
                                ->where('is_active', 1)
                                ->get();
                        }

                        if (!empty($nonAcademicCourseIds)) {
                            $res['nonacademic_courses'] = Course::whereIn('id', $nonAcademicCourseIds)
                                ->where('is_active', 1)
                                ->get();
                        }
                    }
