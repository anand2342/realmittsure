...
class UserSubscriptionController extends Controller
{
    ...
    public function subscription(Request $request)
    {
        ...
        if (!empty($this->data['data']['upgradedPlan'])) {
            ...
            session()->forget('plan_id'); // Remove session if not present
        }
    
        if (!empty($this->data['data']['recomendedPlan'])) {
            session(['recomendedPlanId' => $this->data['data']['recomendedPlan']->id]);
        } else {
            session()->forget('recomendedPlanId'); // Remove session if not present
        }
    
        return view('userPortal.userSubscription.subscription', $this->data);
    }
