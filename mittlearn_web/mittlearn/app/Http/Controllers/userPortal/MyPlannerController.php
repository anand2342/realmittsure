...
class MyPlannerController extends Controller
{
    ...
    public function planner(Request $request)
    {
        try {
            ...
            $classId  = StudentDetails::where('user_id', Auth::id())
                ->value('class');
            // $schoolAssignedDigitalContent = SchoolAssignedDigitalContent::where('school_id', $schoolId)->where('class_id', $classId)->first();
            // if (isset($schoolAssignedDigitalContent)) {
            ...
            // dd($classes);
            if ($classes->isEmpty()) {
                return view('userPortal.myPlanner.index');
            }
            ...
            if ($plannerType == 'daily') {
                ...
                return view('userPortal.myPlanner.index', compact('plannerType', 'schoolId', 'classes', 'totalPlannerDays', 'dayWiseData', 'subjects', 'weekDays', 'allDates', 'totalDays', 'startDate', 'endDate'));
            } elseif ($plannerType == 'weekly') {
                // Get planner data grouped by week
                $plannerDates = Planner::where('class_id', $classId)
                    ->whereIn('subject_id', $schoolAssignedSubjects)
                    ->selectRaw('MIN(start_date) as start_date, MAX(completion_date) as completion_date')
                    ->first();
                ...
                return view('userPortal.myPlanner.index', compact('plannerType', 'schoolId', 'classes', 'weeks', 'weekWiseData', 'subjects'));
                ...
                return view('userPortal.myPlanner.monthly_planner', compact('classes', 'classPlannerData', 'startOfMonth', 'endOfMonth'));
            } else {
                return view('userPortal.myPlanner.index');
            }
        } catch (\Exception $e) {
    ...
    public function plannerChapterListing(Request $request, $slug, $id)
    {
        ...
        return view('userPortal.myPlanner.courses-chapter-listing', $this->data);
    }
}
