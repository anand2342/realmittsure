...
class ErpDataSyncController extends Controller
{
    ...
    public function studentsIndex(Request $request)
    {
        ...
        $searchName = $request->name;
        if ($selectedSchool) {
            $students = DB::connection('erp')->table('all_user')
                ...
                ->map(function ($student) {
                    // Match session for 2023-2024 or 2024-2025
                    $extra = DB::connection('erp')->table('student_extra_value')
                        ->where('addNumber', $student->name)
                        ->whereIn('session', ['2023-2024', '2024-2025'])
                        ->first();

                    if ($extra && $extra->classId) {
                        $className = DB::connection('erp')->table('class')
                            ->where('id', $extra->classId)
                            ->value('name');
                    } else {
                        $className = null;
                    }

                    // Attach only if session matches
                    if ($extra) {
                        $student->class_name = $className ?? 'N/A';
                        return $student;
                    }

                    return null; // filter out this student later
                })
                ->filter() // remove null entries (students without matching session)
