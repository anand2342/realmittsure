...
class ErpDataSyncController extends Controller
{
	...
	public function schoolsIndex(Request $request)
	{
		...
		$query = DB::connection('erp')->table('all_user')
			...
			->orderByDesc('all_user.update_time');
	...
	public function teachersIndex(Request $request)
	{
		...
		if ($selectedSchool) {
			$teachers = DB::connection('erp')->table('employeeaddmission')
				->where('sub_category', 'Teacher')
				->where('status', 'active')
				...
				->map(function ($teacher) {
					// Match with all_user to get password
					$user = DB::connection('erp')->table('all_user')
						->where('name', $teacher->empusername)
						->first();

					$teacher->password = $user->password ?? null;
					...
					if (!empty($classIds)) {
						// Get class names
						$classes = DB::connection('erp')->table('class')
							->whereIn('groupid', $classIds)
							->where('schid', $teacher->schid)
							->pluck('name')
							->toArray();
	...
	public function teachersIndexOld(Request $request)
	{
		// Populate school dropdown
		$this->data['schools'] = User::whereNotNull('erp_schid')
			->select('erp_schid', 'name')
		...
		if ($selectedSchool) {
			$students = DB::connection('erp')->table('registration1')
				...
				->map(function ($student) {
					// dump( $student->classid); 
					$className = DB::connection('erp')->table('class')
						->where('schid', $student->schid)
