...
$(document).ready(function () {
    ...
    const classSelectRegister = $("#className");
    ...
    schoolSearchInput.on("blur", function () {
        setTimeout(() => {
            if (schoolSelect.val() === "" && schoolSearchInput.val() !== "") {
                // Treat as new school - show all classes and series
                classSelectWrapper.show();
                seriesSelectWrapper.show();
                fetchClasses("all", classSelectRegister);
                fetchSeries("all", null, seriesSelect);
            }
        }, 200);
    });
    ...
    schoolSelect.change(function () {
        ...
        classSelectRegister
            .empty()
            .append('<option value="" selected>Select Class</option>');
        seriesSelect
            .empty()
            .append('<option value="" selected>Select Series</option>');

        if (selectedSchool) {
            classSelectWrapper.show();
            seriesSelectWrapper.show();

            if ($(this).find("option:selected").data("classes")) {
                // If classes are embedded in data attribute
                const classes = $(this).find("option:selected").data("classes");
                classes.forEach((classItem) => {
                    classSelectRegister.append(
                        `<option value="${classItem.id}">${classItem.name}</option>`
                    );
                });
            } else {
                // Fetch classes from server
                fetchClasses(selectedSchool, classSelectRegister);
            }
        } else {
    ...
    classSelectRegister.change(function () {
        ...
        if (selectedSchool != null && selectedClass) {
            fetchSeries(selectedSchool, selectedClass, seriesSelect);
        }
        fetchSeries(selectedSchool, selectedClass, seriesSelect);
    });
    ...
    $("form").submit(function (e) {
        if (schoolSelect.val() === "" && schoolSearchInput.val() !== "") {
            // Create hidden input for custom school name
            $("<input>")
                .attr({
                    type: "hidden",
    ...
    function fetchClasses(schoolId, classSelectRegister) {
        $.ajax({
            url: "/get-classes",
            method: "GET",
            ...
            if (
                ...
                const discountMessage =
                    remaining_courses === 0
                        ...
                        <p class='w-100'> You got extra <span><b>${discount.type === "flat"
                            ? `₹${discount.value}`
                            : `${discount.value}%`
                        }</b></span> off on your order.</p>`
                        : `
                        <span>Get <b>${discount.type === "flat"
                            ? `₹${discount.value}`
                            : `${discount.value}%`
                        }</b> Discount</span>
